...
def mini_Backwardable_test(i):
    ...
    while True:
        print('\ni =',i)
        command = prompt.for_char('Enter 1-character Command: [n]ext, [p]rev, [c]lear, or [q]uit', legal= 'npcq')
        try:
            if command == 'n':
                print(' ',next(i))
            elif command == 'p':
                ...
                clear(i)
            else: # must be c: prompt allows only npcq
                break
        except:
...
def Backwardable_test(i,command_sequence):
    answer = ''
