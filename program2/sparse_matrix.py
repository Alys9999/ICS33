...
class Sparse_Matrix:
    ...
    def __call__(self,n_row,n_col):
        if type(n_row) not in [int] or type(n_col) not in [int] or n_row<=0 or n_col<=0:
            ...
            self.__dict__['rows']=n_row
            self.__dict__['cols']=n_col
            self.__dict__['matrix']=dict((x,self.matrix[x]) for x in self.matrix.keys() if x[0]<self.rows and x[1]<self.cols)
    ...
    def __setattr__(self,name,value):
        if name not in self.__dict__.keys() and name in ['rows','cols','matrix']:
            self.__dict__[name]=value
        else:
            raise AssertionError('Sparse_Matrix.__setattr__: Cannot add new attributes to an object')
